...
xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        ...
        if (kara["status"] === true) {
            ...
            document.getElementById("ip").innerHTML = "<p>mc.karanteeni.net</p>" + "<p id='numip'>" + kara["hostname"] + ":" + kara["port"] + "</p>" + "<button style='margin: auto; margin-top: 10px;' class='btn btn-outline-dark btn-tooltip show' data-toggle='tooltip' data-placement='top' title='Paina tästä kopioidaksesi IP-osoite.' onclick='copyIp(document.getElementById(\"numip\"));'><i class='fas fa-copy'></i></button>";
